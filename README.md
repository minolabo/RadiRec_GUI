...
2. `RadiRec_GUI.exe` を実行してください。
   *   ※ `ffmpeg.exe` を同一フォルダに置くか、システム環境変数(PATH)を通してください。ない場合は下記手順に従ってダウンロードしてください。
...
*   **URLから録音**: 番組ページのURL（例: `https://radiko.jp/#!/ts/JORF/20260208000000`）を貼り付けて録音します。
    *   **時間指定機能**: 「時間指定を行う」を有効にすると、番組開始から「X分後」から「Y分間」だけを切り出して録音できます。
*   **日時指定録音**: 放送局と開始日時、録音時間（分）を手動で指定して録音します。

### 設定・オプション
*   **プレミアムログイン**: エリア外やタイムフリーなどradikoプレミアム会員機能を利用する場合に入力します。
*   **ファイル名規則**: 保存するファイル名のフォーマットを指定できます。
    *   `{DATE}`: 録音開始日の日付 (YYYYMMDD)
    *   `{TIME}`: 録音開始時の時間 (HHMM)
    *   `{TITLE}`: 番組タイトル
    *   `{STATION}`: 放送局ID
    *   ※URL録音で時間指定を行った場合、`{DATE}` と `{TIME}` は**オフセット後の実際の録音開始時刻**が反映されます。
